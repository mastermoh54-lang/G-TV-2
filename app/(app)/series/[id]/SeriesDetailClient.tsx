...
export function SeriesDetailClient({ seriesId }: { seriesId: string }) {
  ...
  const fav = isFav("series", Number(seriesId));
  ...
  return (
    <div className="min-h-screen bg-ink-950 text-white p-3 sm:p-6 space-y-6">
      ...
      <DetailHero
        ...
        title={title}
        fav={fav}
        onToggleFav={() => toggleFav("series", { id: Number(seriesId), name: cleanName(title), poster: info?.cover })}
      >
            ...
            {episodes.map((ep: Episode) => {
              ...
              return (
                ...
                >
                  ...
                  <Link
                    href={`/watch?type=series&id=${ep.id}&ext=${ext}&title=${encodeURIComponent(`${cleanName(title)} ·${epTitle}`)}&series=${seriesId}${resume > 15 ? `&resume=${Math.floor(resume)}` : ""}`}
