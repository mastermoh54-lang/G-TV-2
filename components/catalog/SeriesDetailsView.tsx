...
function EpisodeImage({ ep, seriesTitle, tmdbId, seasonKey, fallbackCover }: any) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/episode-image?tmdbId=${tmdbId || ""}&show=${encodeURIComponent(seriesTitle)}&season=${cleanSeason}&episode=${ep.episode_num}`)
      .then((res) => res.json())
      .then((data) => { if (isMounted && data?.imageUrl) setImgSrc(data.imageUrl); })
      .catch(() => {});
...
export default function SeriesDetailPage() {
  ...
  const seasons = useMemo(() => {
    ...
    return Object.keys(episodesBySeason).filter((k) => (episodesBySeason[k] ?? []).length > 0).sort((a, b) => {
  ...
  const activeSourceUrl = useMemo(() => {
    ...
    return [`/api/show?id=${activeEpisode.id}&ext=${activeEpisode.container_extension || "mp4"}&cb=${Date.now()}`];
  }, [activeEpisode]);
  ...
  const handleDoubleTap = () => {
    const now = Date.now();
    if (now - lastTapRef.current < 300) handleFullscreenLandscape();
    ...
    <div className="min-h-screen bg-ink-950 text-white p-3 sm:p-6 space-y-6">
      ...
      <DetailHero backdrop={info?.backdrop_path?.[0] || info?.backdrop} poster={info?.cover} title={title} fav={fav} onToggleFav={() => toggleFav("series", { id: Number(id), name: cleanName(title), poster: info?.cover })}>
        <div className="space-y-4 max-w-4xl">
          ...
          {(info?.plot || info?.description) && <p className="text-sm leading-relaxed text-fog-300 font-light">{info.plot || info.description}</p>}

          {castList.length > 0 && (
            <div className="pt-3 border-t border-white/10 space-y-2">
              <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2"><User className="w-3.5 h-3.5 text-iris-400" /> Casting / Acteurs</h3>
              <div className="flex flex-wrap gap-2.5 pt-1">{castList.slice(0, 8).map((actor: string, idx: number) => <FlipActorCard key={idx} name={actor} />)}</div>
            </div>
          )}
        </div>
        ...
        {seasons.length > 0 && (
          <div className="mt-8 no-scrollbar flex gap-2 overflow-x-auto pb-2 border-b border-white/5">
            {seasons.map((s) => {
              ...
              return <button key={s} onClick={() => { setSeasonKey(s); setActiveEpisode(null); }} className={cn("shrink-0 rounded-full px-4 py-1.5 text-sm font-medium transition-colors", s === activeSeasonKey ? "bg-iris-400 text-ink-950 font-bold" : "bg-ink-800 text-fog-400 hover:bg-ink-700 hover:text-white")}>{label}</button>;
            })}
          </div>
        )}
          ...
          {activeEpisode && (
            <div className="w-full lg:w-1/2 shrink-0 space-y-3 bg-ink-900 border border-white/10 rounded-2xl p-4 sticky top-6 shadow-2xl z-30">
              <div className="flex items-center justify-between px-1">
                <h2 className="text-xs font-bold uppercase tracking-wider text-iris-400 truncate max-w-[70%]">S{activeEpisode.season || activeSeasonKey}E{activeEpisode.episode_num} - {cleanName(activeEpisode.title || "")}</h2>
                <div className="flex items-center gap-1">
                  <button onClick={handleFullscreenLandscape} className="text-fog-400 hover:text-white p-1 rounded-lg hover:bg-white/5 transition-colors" title="Plein Écran Horizontal"><Maximize className="w-4 h-4" /></button>
              ...
              return (
                <div key={ep.id} onClick={() => setActiveEpisode(ep)} className={cn("group flex items-center gap-4 rounded-xl border p-2.5 transition-colors cursor-pointer", isSelected ? "bg-ink-800 border-iris-500/50 shadow-md" : "bg-ink-850/60 border-white/5 hover:bg-ink-800")}>
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="flex items-center gap-2 font-medium text-sm"><span className="text-fog-500">{ep.episode_num}.</span><span className="truncate group-hover:text-iris-300 transition-colors">{cleanName(epTitle)}</span></p>
