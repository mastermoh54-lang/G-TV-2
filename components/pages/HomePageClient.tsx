...
import { NavTile, ContinueTile } from "@/components/catalog/Bento";
  ...
  return (
    <div className="group relative mb-8 flex h-[65vh] min-h-[450px] w-full items-end overflow-hidden sm:h-[75vh] sm:min-h-[550px] sm:rounded-b-[3rem]">
      ...
      <div className="relative z-10 w-full max-w-4xl px-5 pb-16 sm:px-12 sm:pb-24">
        ...
        <div className="mb-4 flex items-center gap-3 text-xs font-semibold sm:text-sm animate-in fade-in duration-1000 delay-150">
          {activeItem.rating && (
            <span className="flex items-center gap-1 rounded-md bg-amber-500/20 px-2 py-1 text-amber-400 backdrop-blur-md">
              <Star className="h-3.5 w-3.5 fill-current" /> {activeItem.rating.toFixed(1)}
            </span>
          )}
          {activeItem.year && <span className="text-zinc-300">{activeItem.year}</span>}
          {activeItem.meta && <span className="text-zinc-400">&bull; {activeItem.meta}</span>}
        </div>

        {activeItem.plot && (
          <p className="mb-8 line-clamp-3 max-w-2xl text-sm leading-relaxed text-zinc-300 sm:text-base animate-in fade-in duration-1000 delay-300">
            {activeItem.plot}
          </p>
        )}
        ...
        <div className="flex items-center gap-3 animate-in fade-in duration-1000 delay-500">
          <Link
            href={activeItem.playHref || activeItem.detailHref}
            className="flex items-center gap-2 rounded-xl bg-white px-6 py-3.5 font-bold text-black transition-transform hover:scale-105 active:scale-95 shadow-xl shadow-white/10"
          >
            <Play className="h-5 w-5 fill-current" /> Lecture
          </Link>
          <Link
            href={activeItem.detailHref}
            className="flex items-center gap-2 rounded-xl bg-white/20 px-6 py-3.5 font-bold text-white backdrop-blur-md transition-colors hover:bg-white/30 active:scale-95"
          >
  ...
  return (
    <div className="bg-[#0b0c10] min-h-screen text-white pb-10">
      ...
      {heroLoading ? (
        ...
        <MainHeroSlider items={heroItems as any} />
      ) : (
        <div className="pt-24" />
      )}
      ...
      <div className="space-y-8 px-5 sm:px-8 relative z-10 -mt-8 sm:-mt-12">
        <div className="grid auto-rows-[168px] grid-cols-2 gap-4 lg:grid-cols-4 drop-shadow-2xl">
          {cw.length > 0 ? (
            <ContinueTile items={cw} className="col-span-2 row-span-1" />
          ) : (
            <NavTile
              href="/movies"
              title={t("Home.movies")}
              ...
              icon="film"
              tint="iris"
              className="col-span-2 row-span-1 border border-white/5"
            />
          )}
